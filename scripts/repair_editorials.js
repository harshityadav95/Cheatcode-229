...
function updatePublicDetailsJson(leetcodeNum, editorialHtml) {
  ...
  if (fs.existsSync(detailJsonPath)) {
    try {
      ...
      console.log(`Updated details json for #${leetcodeNum}`);
    } catch (e) {
...
async function repairProblem(browser, problem) {
  const { slug, folder, leetcodeNum } = problem;
  const localEditorialPath = path.join(PROBLEMS_DIR, folder, 'editorial.json');
  
  console.log(`Repairing/Re-checking editorial for ${slug} (LeetCode #${leetcodeNum})...`);
  ...
  try {
    ...
    // Use a longer wait (6s) to ensure elements render completely
    ...
    if (!containerExists) {
      ...
      const isPremium = pageText.includes('subscribe to premium') || 
                        ...
                        pageText.includes('to view this solution you must subscribe');
                        
      console.log(`-> Still not found. Premium: ${isPremium}`);
    ...
    console.log(`-> Free Editorial FOUND! Scraping...`);
  ...
  <details ${isFirst ? 'open' : ''} name="${playgroundId}" class="border-b border-slate-200 dark:border-slate-800 last:border-b-0">
    ...
    console.error(`Error repairing ${slug}:`, err);
  } finally {
    await page.close();
  }
}
...
async function main() {
  ...
  for (const folder of folders) {
    const p = path.join(PROBLEMS_DIR, folder, 'problem.json');
    if (fs.existsSync(p)) {
      try {
        const problemData = JSON.parse(fs.readFileSync(p, 'utf8'));
        const leetcodeNum = problemData.leetcode;
        let slug = '';
        if (problemData.references) {
          const mainProbRef = problemData.references.find(ref => ref.kind === 'problem');
          if (mainProbRef && mainProbRef.url) {
            const match = mainProbRef.url.match(/https:\/\/leetcode\.com\/problems\/([^/]+)/);
            if (match) slug = match[1];
          }
        }
        if (!slug) slug = folder.replace(/^\d{3}-/, '');
        
        const editorialPath = path.join(PROBLEMS_DIR, folder, 'editorial.json');
        if (fs.existsSync(editorialPath)) {
          const edData = JSON.parse(fs.readFileSync(editorialPath, 'utf8'));
          if (edData.available === false && edData.premium === false) {
            toRepair.push({ folder, slug, leetcodeNum });
          }
        }
      } catch (e) {}
    }
  ...
  console.log(`Found ${toRepair.length} false-negative or not-found editorial files to repair.`);
  ...
  try {
    for (const problem of toRepair) {
      await repairProblem(browser, problem);
    }
  } catch (e) {
    console.error("Repair batch error:", e);
  } finally {
    await browser.close();
    console.log("Browser closed. Repair process complete!");
  }
}
